...
export const Route = createFileRoute("/tenant-role-menu")({
  head: () => ({
    meta: [
      { title: "Role Menu Assign — Akira School ERP" },
      { name: "description", content: "Assign your school's own sidebar menus and action permissions to roles." },
    ],
  }),
  component: TenantRoleMenuPage,
});
...
function TenantRoleMenuPage() {
  ...
  useEffect(() => {
    if (!roleKey) return;
    setLoading(true);
    apiFetch(`/api/TenantRoleMenu/${roleKey}`)
  ...
  const toggle = (menuKey: string, key: PermissionKey) => {
    setMatrix((rows) =>
      rows.map((row) => {
        if (row.tenantMenuKey !== menuKey) return row;
  ...
  const save = async () => {
    setSaving(true);
    try {
      await apiFetch("/api/TenantRoleMenu/assign", {
        method: "POST",
        body: JSON.stringify({
          akiraRoleKey: roleKey,
          items: matrix.map((r) => ({
            tenantMenuKey: r.tenantMenuKey,
            canView: r.canView,
            canAdd: r.canAdd,
            canEdit: r.canEdit,
  ...
  const sections = Array.from(new Set(matrix.map((m) => m.parentMenuName ?? "Menu")));
              ...
              {loading ? (
                ...
                sections.map((section) => (
                  <Fragment key={section}>
                    ...
                    {matrix
                      .filter((m) => (m.parentMenuName ?? "Menu") === section)
                      .map((row) => {
                        const allChecked = PERMISSION_COLUMNS.every((c) => row[c.key]);
                        return (
                          <TableRow key={row.tenantMenuKey} className="border-border">
                            <TableCell className="text-sm font-medium">{row.menuName}</TableCell>
                            {PERMISSION_COLUMNS.map((c) => (
                              <TableCell key={c.key} className="text-center">
                                <Checkbox
                                  checked={row[c.key]}
                                  onCheckedChange={() => toggle(row.tenantMenuKey, c.key)}
                                />
                              </TableCell>
                            ))}
                            <TableCell className="text-center">
                              <Checkbox
                                checked={allChecked}
                                onCheckedChange={(v) => toggleRowAll(row.tenantMenuKey, !!v)}
